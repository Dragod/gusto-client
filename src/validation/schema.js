...
const schema = Joi.object({
	name: Joi.string().min(3).max(100).trim().empty('').messages({
	...
	description: Joi.string().min(10).max(500).trim().empty('').messages({
	...
	price: Joi.number().integer().min(1).max(99).messages({
	...
	is_pizza: Joi.number().valid(0, 1).messages({
